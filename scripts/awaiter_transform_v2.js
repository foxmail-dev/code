...
function transformAwaiterCall(path) {
  ...
  if (asyncFunction) {
    ...
    if (t.isReturnStatement(parentReturn)) {
      ...
      if (t.isFunctionExpression(outerFunc.node) || t.isFunctionDeclaration(outerFunc.node)) {
 ...
 */
function convertStateMachineToAsync(generatorFunc, switchStmt, stateVarName, thisArg) {
  ...
  const processedStatements = processState(currentState, stateMap, visitedStates, stateVarName);
...
function processState(stateLabel, stateMap, visitedStates, stateVarName) {
  ...
  for (const stmt of consequent) {
    const result = processStatement(stmt, stateMap, visitedStates, stateVarName);
    if (result) {
...
function processStatement(stmt, stateMap, visitedStates, stateVarName) {
  ...
  if (t.isReturnStatement(stmt) && stmt.argument && t.isArrayExpression(stmt.argument)) {
    ...
    if (t.isConditionalExpression(stmt.argument)) {
      return processConditional(stmt.argument, stateMap, visitedStates, stateVarName);
    }
  }
  ...
  if (t.isExpressionStatement(stmt) && t.isAssignmentExpression(stmt.expression)) {
    ...
    if (t.isCallExpression(expr.right) && 
...
function processConditional(condExpr, stateMap, visitedStates, stateVarName) {
  ...
  // Parse both branches
  const thenStmts = parseArrayReturn(consequent, stateMap, visitedStates, stateVarName);
  const elseStmts = parseArrayReturn(alternate, stateMap, visitedStates, stateVarName);
  
  // Create if statement
  const ifStmt = t.ifStatement(
    test,
    t.blockStatement(Array.isArray(thenStmts) ? thenStmts : [thenStmts].filter(Boolean)),
    elseStmts ? t.blockStatement(Array.isArray(elseStmts) ? elseStmts : [elseStmts].filter(Boolean)) : null
  );
...
function handleTryBlock(stmt, stateMap, visitedStates, stateVarName) {
  ...
  if (t.isArrayExpression(args)) {
    ...
    for (let label = startLabel + 1; label < endLabel; label++) {
      const stateStmts = processState(label, stateMap, new Set(visitedStates), stateVarName);
...
function main() {
  ...
  function transformFile(inputPath, outputPath) {
    ...
    if (outputCode.startsWith('var _fn = ') || outputCode.startsWith('function _fn(')) {
      // Extract the function body
      const match = outputCode.match(/(?:var _fn = |function _fn\(\) \{)([\s\S]*?)(?:\};?\s*$|\}$)/);
      if (match) {
