  ...
  return {
    ...
    visitor: {
      CallExpression(path) {
        ...
        if (!t.isFunctionExpression(generatorFunc) || !generatorFunc.generator) {
          return;
        }
  ...
  for (const stmt of statements) {
    if (t.isReturnStatement(stmt) && stmt.argument && 
      ...
      if (t.isMemberExpression(switchStmt.discriminant) &&
          t.isIdentifier(switchStmt.discriminant.object, { name: stateVarName }) &&
...
function processState(stateLabel, stateMap, visitedStates, stateVarName) {
  ...
  for (const stmt of consequent) {
      ...
      if (t.isArrayExpression(arg)) {
        ...
        switch (stateCode) {
          ...
          case 1: // Yield/Await - expects [1, promise]
            ...
            statements.push(t.expressionStatement(t.awaitExpression(elements[1])));
            break;
          ...
          case 3: // Goto/jump - [3, targetState]
            ...
            if (t.isNumericLiteral(targetState)) {
              ...
              const targetResult = processState(targetState.value, stateMap, visitedStates, stateVarName);
              if (targetResult) {
    ...
    const transformed = transformStatement(stmt, stateMap, visitedStates, stateVarName);
    if (transformed) {
...
function transformTryStatement(tryStmt, stateMap, visitedStates, stateVarName) {
  ...
  if (handler) {
    const catchResult = processStatements(handler.body.body, stateMap, visitedStates, stateVarName);
    catchClause = t.catchClause(handler.param, t.blockStatement(catchResult));
  }
  
  let finallyBlock = null;
  if (finalizer) {
    const finallyResult = processStatements(finalizer.body, stateMap, visitedStates, stateVarName);
    finallyBlock = t.blockStatement(finallyResult);
  }
  
  const tryResult = processStatements(tryStmt.block.body, stateMap, visitedStates, stateVarName);
  
  return t.tryStatement(
    t.blockStatement(tryResult),
    catchClause,
    finallyBlock
  );
}
...
function transformIfStatement(ifStmt, stateMap, visitedStates, stateVarName) {
  const consequent = t.isBlockStatement(ifStmt.consequent)
    ? processStatements(ifStmt.consequent.body, stateMap, visitedStates, stateVarName)
    : [transformStatement(ifStmt.consequent, stateMap, visitedStates, stateVarName)].filter(Boolean);
  
  let alternate = null;
  if (ifStmt.alternate) {
    alternate = t.isBlockStatement(ifStmt.alternate)
      ? processStatements(ifStmt.alternate.body, stateMap, visitedStates, stateVarName)
      : [transformStatement(ifStmt.alternate, stateMap, visitedStates, stateVarName)].filter(Boolean);
  }
...
function generateCode(node) {
  try {
    return babel.transformFromAstSync(t.cloneNode(node), null, {
...
if (require.main === module) {
  ...
  if (args.length < 1) {
    console.error('Usage: node awaiter_transform_strict.js <input-file> [output-file]');
    console.error('       node awaiter_transform_strict.js --dir <input-dir> <output-dir>');
    process.exit(1);
  }
